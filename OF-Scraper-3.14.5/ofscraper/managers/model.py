...
class ModelManager:
    ...
    def prepare_scraper_activity(
        ...
        elif getattr(settings.get_args(), "gui", False):
            # GUI MODE: Model selection is already controlled by the GUI via
            # args.usernames — skip the interactive terminal prompt entirely.
            # (InquirerPy/prompt_toolkit needs a real TTY; there is none in a GUI
            # process, so termios.tcsetattr raises ERRNO 5 Input/output error.)
            log.info("GUI mode: bypassing terminal model-selection prompt.")
            final_selection = self._select_models_scraper()
        else:
