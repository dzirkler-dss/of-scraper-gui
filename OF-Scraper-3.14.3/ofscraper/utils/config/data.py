...
def get_filesize_max(config=None):
    ...
    try:
        ...
        final_size = (
            size if size else of_env.getattr("FILE_SIZE_MAX_DEFAULT")
        )
        return parse_size(str(final_size)) if str(final_size).strip() else 0
...
def get_filesize_min(config=None):
    ...
    try:
        ...
        final_size = (
            size if size else of_env.getattr("FILE_SIZE_MIN_DEFAULT")
        )
        return parse_size(str(final_size)) if str(final_size).strip() else 0
    except (ValueError, TypeError):
        return 0
