...
async def process_tasks_get_posts_for_labels(generators, labels):
    ...
    queue = asyncio.Queue()
    # Limit concurrent label fetches to avoid hitting the OF API rate limit.
    # Running 40+ label requests in parallel reliably triggers 429/empty responses.
    _label_sem = asyncio.Semaphore(5)

    async def producer(gen):
        try:
            async with _label_sem:
    ...
    # Warn about labels that came back empty — usually means rate-limit exhaustion
    for _lbl_key, _lbl_val in responseDict.items():
        if len(_lbl_val.get("posts", [])) == 0:
            log.warning(
                f"[WARN] Label '{_lbl_val.get('name', _lbl_key)}'"
                f" (id={_lbl_key}) returned 0 posts after all retries"
                f" — possible rate-limit failure; content count may be lower than expected"
            )
